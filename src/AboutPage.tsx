...
const VALUES = [
  ...
  { name: 'Results', desc: "Client success is the ultimate measure of success. Period." },
    ...
    <div className="main-wrapper min-h-screen" style={{ background: '#0c0c0c' }}>
      ...
      >
        <div className="max-w-6xl mx-auto grid md:grid-cols-2 gap-12 md:gap-20 items-start">
          {/* Photos */}
          <FadeIn x={-40} y={0} duration={0.8} className="flex gap-4 items-end">
            <img
              ...
              alt="Xan Orchid"
              className="flex-1 object-cover object-top rounded-3xl"
              style={{ aspectRatio: '2/3' }}
            />
            <img
              src="https://static.wixstatic.com/media/b80b05_9ce31db91e124350895756cffa3ad5f3~mv2.jpg/v1/crop/x_200,y_0,w_1503,h_1815/fill/w_600,h_800,al_c,q_90,enc_avif,quality_auto/4I5A1372_JPG.jpg"
              alt="Xan Orchid"
              className="flex-1 object-cover object-top rounded-3xl mt-12"
              style={{ aspectRatio: '3/4' }}
            />
          </FadeIn>

          {/* Bio text */}
          <FadeIn x={40} y={0} duration={0.8} delay={0.1} className="flex flex-col gap-6">
            <h2
              className="font-black uppercase leading-none"
              style={{ fontSize: 'clamp(2rem, 5vw, 60px)', color: '#0c0c0c' }}
            >
              Xan Orchid
            </h2>
            <p
            ...
            {[
              ...
              "In her freelance work, Xan helps clients with branding, website creation, social media marketing, logo design, and template design. She has also built her own startups: GoGal (an invite-only platform for women to connect and travel safely) and Astro Trips, an NFT project.",
            ].map((para, i) => (
              <p
                key={i}
            ...
            <div className="mt-2">
              <Link
                to="/portfolio"
                className="inline-block rounded-full px-8 py-3 font-medium uppercase
                           tracking-widest text-sm text-white transition-opacity hover:opacity-90"
                style={{
                  background:
      ...
      {/* Values — dark section */}
      <section
        className="-mt-10 z-10 relative rounded-t-[40px] sm:rounded-t-[50px] md:rounded-t-[60px]
                   px-5 sm:px-8 md:px-10 py-20 sm:py-24 md:py-32"
      ...
      >
        ...
        <div className="max-w-4xl mx-auto grid sm:grid-cols-2 md:grid-cols-3 gap-5 sm:gap-6">
          {VALUES.map((val, i) => (
            <FadeIn key={val.name} delay={i * 0.1} y={20}>
              <div
                className="rounded-2xl p-6 sm:p-8 h-full"
                style={{ border: '1px solid rgba(215,226,234,0.12)' }}
              >
                <h3 className="text-[#D7E2EA] font-black uppercase text-lg mb-3">
                  {val.name}
                </h3>
                <p
